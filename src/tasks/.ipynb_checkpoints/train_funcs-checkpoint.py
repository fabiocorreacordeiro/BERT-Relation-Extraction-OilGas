...
def evaluate_results(net, test_loader, pad_id, cuda):
    ...
    print(list(itertools.chain.from_iterable(true_labels)), '\n', list(itertools.chain.from_iterable(out_labels)))
    results = {
        "accuracy": accuracy,
        #"precision": precision_score(true_labels, out_labels),
        #"recall": recall_score(true_labels, out_labels),
        "f1": f1_score(list(itertools.chain.from_iterable(true_labels)), list(itertools.chain.from_iterable(out_labels)), average='macro')
    }
