...
def process_text(text, mode='train'):
    ...
    for i in range(int(len(text)/4)):
        sent = text[4*i].replace('\n','')
        relation = text[4*i + 1].replace('\n','')
        comment = text[4*i + 2].replace('\n','')
        blank = text[4*i + 3]
        
        #print(sent, relation, comment, blank)
        # check entries
        #if mode == 'train':
        #    assert int(re.match("^\d+", sent)[0]) == (i + 1)
        #else:
        #    assert (int(re.match("^\d+", sent)[0]) - 8000) == (i + 1)
        assert re.match("^Comment", comment), sent
        assert len(blank) == 1
...
def preprocess_semeval2010_8(args):
    ...
    df_all = df_all[~df_all['relations'].str.contains('Todo-Parte')]
    ...
    df_all['relations_id'] = df_all.progress_apply(lambda x: rm.rel2idx[x['relations']], axis=1)
    save_as_pickle('relations.pkl', rm)
    
    sss = StratifiedShuffleSplit(n_splits=1, test_size=0.20, random_state=42)
    train_idx, test_idx = next(sss.split(df_all, df_all['relations_id']))
...
def load_dataloaders(args):
    ...
    if args.task == 'semeval':
        ...
        train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=False, \
                                  num_workers=1, collate_fn=PS, pin_memory=False)
