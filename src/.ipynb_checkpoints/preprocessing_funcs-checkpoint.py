...
class pretrain_dataset(Dataset):
    def __init__(self, args, D, batch_size=None):
        ...
        #self.df['e1e2'] = self.df['e1'] + self.df['e2']
        #self.df.set_index(['e1e2'],inplace=True)
        #self.df.reset_index(['e1e2'], inplace=True)

        #with open('./data/dict_pos_examples.pkl','rb') as f:
        #    self.dict_pos_examples = pickle.load(f)        
        #with open('./data/dict_neg_examples.pkl','rb') as f:
        #    self.dict_neg_examples = pickle.load(f)        
        ...
        if os.path.isfile(tokenizer_path):
            ...
            if args.model_no == 2:
                ...
                if args.model_size == 'neuralmind/bert-base-portuguese-cased':
    ...
    def __getitem__(self, idx):
        ...
        ### implements noise contrastive estimation
            ...
            #pos_idxs = []
            #pool = self.dict_pos_examples[(e1,e2)]
            ...
            #neg_idxs = []
            #pool = self.dict_neg_examples[(e1,e2)]
            #Q = 1/(len(pool)+1)
...
def load_dataloaders(args, max_length=50000):
    ...
    if not os.path.isfile("./data/D.pkl"):
        ...
        with open(args.pretrain_data, "r", encoding="utf8") as f:
            ...
            N = 100000 #len(text)
            #N = len(text)
            for line in tqdm(text[:N], total=N):
                #text = text[:1500] # restrict size for testing
                line = process_textlines(line)
                for sent in re.split(r'[.?!]', line):
                    D.extend(create_pretraining_corpus(sent, nlp))
                #logger.info("Length of text (characters): %d" % len(line))
                ...
                #    for text_sent in sent_tokenize(text_chunk):
                #        D.extend(create_pretraining_corpus(text_sent, nlp))
        ...
        D = load_pickle("D.pkl")
        #D = random.sample(D, 200000)
        logger.info("Loaded pre-training data from saved file. Size = %d " % len(D))
