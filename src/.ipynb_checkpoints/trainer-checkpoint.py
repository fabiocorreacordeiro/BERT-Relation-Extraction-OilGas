...
def train_and_fit(args):
    ...
    train_loader = DataLoader(train_loader, batch_size=1, num_workers=12, shuffle=True)
    ...
    logger.info("Starting training process with %d samples" % len(train_loader))
    pad_id = tokenizer.pad_token_id
    mask_id = tokenizer.mask_token_id
    
    update_size = len(train_loader)//100
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for i, data in tqdm(enumerate(train_loader, 0), total=len(train_loader)):
            #print('Data ===> ',data)
            x, masked_for_pred, e1_e2_start, _, blank_labels, _,_,_,_,_ = map(itemgetter(0),data) #lambda x: x[0]
            #x, masked_for_pred, e1_e2_start, _,blank_labels  = data #lambda x: x[0]
            ...
            if (i % update_size) == (update_size - 1):
                ...
                verbose = False
            #print(lm_logits.shape, blanks_logits.shape, blank_labels.shape)    
